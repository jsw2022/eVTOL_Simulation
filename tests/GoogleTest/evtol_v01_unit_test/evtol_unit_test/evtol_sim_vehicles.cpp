...
//note: need to include <cstdint> for uint8_t 
#include <cstdint>
 ...
 *******************************************************************************/
static void customSleep(int milliseconds) {
#ifdef _WIN32_
    Sleep(milliseconds);
#else
...
public:
    ...
    EvtolVehicle(const std::string& name, double cruiseSpeed, double batteryCapacity,
                 ...
                 : name(name), cruiseSpeed(cruiseSpeed), batteryCapacity(batteryCapacity), 
                   ...
                   disCharge(false), chargeNum(INT_COUNT_0), flightDuration(0), chargingTime(0){ //Note: initial flightDuration
    }
    ...
    void simulateFlight(Charger& charger){
        ...
        if(isFlying){
            ...
            faults           += (float)faultProbability/NUM_MINUTES_HOUR; //note: updat it on release version 
            ...
            if(chargeNum == INT_COUNT_0){
                ...
                if(flightDuration>=vehAirTime){
                    ...
                    //Note: should do simple check to avoid condition blocked by longer flight time range
    ...
    int getTimeToCharge() {
        return (int)(timeToCharge*NUM_MINUTES_HOUR);
    }
    void setTimeCharging(float fltInput) {
        timeCharging = fltInput;
    }
    void setChargeNum(int intInput) {
        chargeNum = intInput;
    }
    void setFlightTime(float fltInput) {
        flightTime = fltInput;
    }
    void setVehChargerId(int intInput) {
...
public:
    ...
    */
    ...
    //Note: need to initialize all parameters    
    SimulateManager() : chargers(NUM_CHARGER_BUF), totalDistance(0), 
                        totalHours(0.0), totalTimeCharging(0.0), 
                        totalFaults(0.0), totalPassengerMiles(0.0){
    }
    ...
    void runSimulation(){
        ...
        for (int i = INT_INDX_0; i < NUM_TOTAL_VEHS; i++){
            ...
            int randomType = rand() % vehicleTypes.size();
            
            const auto& params = vehicleParameters[vehicleTypes[randomType]];
            
            //Note: put valid vehicle data assumption comment
    ...
    void setChargersState(bool blInput) {
        ...
        if (blInput) {//Set available
            for (int i = 0; i < NUM_CHARGER_BUF; i++) {
                chargers[i].release();
            }
        }
        else {//Set occupy
...
private:
    ...
    std::map<std::string, std::tuple<int, int, double, double, int, double>> vehicleParameters = {
